  ...
  crearManga() {
    ...
    if (cont !== 0) { this.presentAlert("Los siguientes campos son requeridos para ingresar a la paguina", "" + camposVacios); }
      ...
      axios.post('http://localhost:8080/api/mangas/add', mangaFormData, {
        ...
        this.presentAlert("Error al crear el manga", error);
...
//campos para la verificacion de compra del carro
  async alquilarCarro(): Promise<boolean> {
    const alert = await this.alertController.create({
      header: '¿Estás seguro de alquilar los mangas del carrito?',
      message: '',
      buttons: [
      {
        text: 'NO',
        role: 'cancel',
        handler: () => { /* aca las funciones de la cancelacion*/ }
      },
      {
        text: 'Si',
        handler: () => {
          ...
          { 'Content-Type': 'application/json' }}
            ...
            console.log('Respuesta del servidor:', response.data);
            this.presentAlert("Mangas alquilados exitosamente", "Disfruta de tu manga y no se te olvide devolverlo.");
            this.getHistorialF();
            this.getMangas();
            this.carro.splice(0, this.carro.length);
            this.ventana = '';
          })
          .catch(error => {
            console.error('Error al intentar alquilar el manga:', error);
